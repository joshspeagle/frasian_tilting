...
# ---------------------------------------------------------------------------
# LEGACY (Phase D) — kept until E.4 cuts over the test_train_smoke regression
# test and learned_eta_demo illustration. New code should use
# ``fit_eta_artifact(config=...)`` above.
# ---------------------------------------------------------------------------

from ..transforms import (  # noqa: E402  (legacy imports below)
    eta_inverse_torch,
)
from .architecture import MonotonicEtaNet  # noqa: E402
...
def _theta_grid_for_D(
    ...
    """LEGACY. Build a per-sample θ-grid `D ± search_mult·σ`."""
...
def _eta_from_mlp(
    ...
    """LEGACY. Run MonotonicEtaNet on (w, |Δ|) and inverse-transform η."""
...
def _compute_pvalue_grid_legacy(
    *,
    model: "MonotonicEtaNet",
    batch: dict,
    scheme_name: str,
    ...
    """LEGACY. (B, N) p-values for the legacy MonotonicEtaNet pipeline."""
    ...
    p_theta = tilted_pvalue(
        theta_per_sample, D.unsqueeze(-1), w.unsqueeze(-1), mu0, sigma,
        eta, statistic_name,
    )
    if not torch.is_grad_enabled():
        out_of_range = ((p_theta < -1e-5) | (p_theta > 1 + 1e-5)).any().item()
        if out_of_range:
            warnings.warn(
                f"p_theta drifted outside [0, 1] in {scheme_name}/{statistic_name}",
                RuntimeWarning,
            )
...
def _compute_loss_legacy(
    *,
    ...
    """LEGACY. Forward pass returning a scalar loss."""
    p_theta, theta_per_sample = _compute_pvalue_grid_legacy(
...
@dataclass
class MonotonicTrainResult:
    """LEGACY. Output of ``fit_monotonic_eta_artifact`` (Phase D)."""
...
# `TrainResult` (singular) used to be the only result class. Phase E
# split it: ``EtaTrainResult`` for the new dual-head loop,
# ``MonotonicTrainResult`` for the legacy MonotonicEtaNet loop. The
# bare ``TrainResult`` symbol is intentionally NOT re-exported — any
# caller that imported it must now name its variant explicitly.
...
def _compute_calibration_report(
    *,
    ...
    """LEGACY. Empirical coverage of the legacy MLP."""
    ...
    with torch.no_grad():
        for i, theta_true in enumerate(theta_true_grid):
            theta_true_t = torch.tensor(theta_true, dtype=torch.float32, device=device)
            for j, w_val in enumerate(w_grid):
                w_t = torch.full((n_reps,), float(w_val), dtype=torch.float32, device=device)
                D = theta_true_t + sigma_t * torch.randn(
                    n_reps, generator=rng, device=device, dtype=torch.float32,
                )
                abs_delta = torch.abs((1.0 - w_t) * (mu0_t - theta_true_t) / sigma_t)
                from ..transforms import delta_transform_torch
                ...
                p = tilted_pvalue(theta_in, D, w_t, mu0_t, sigma_t, eta, statistic_name)
                p = p.clamp(0.0, 1.0)
                for k, a in enumerate(alphas):
                    coverage[i][j][k] = float((p >= a).float().mean().item())
    return {
        "alphas": list(alphas),
...
def fit_monotonic_eta_artifact(
    *,
    ...
    version: str = "v0",
    verbose: bool = True,
) -> "MonotonicTrainResult":
    """LEGACY (Phase D). Train the old MonotonicEtaNet end-to-end.

    Kept so ``test_train_smoke.py`` and ``learned_eta_demo.py`` continue
    to load existing v0_smoke checkpoints; new code should use
    ``fit_eta_artifact(config=...)`` above. Removed in E.4.
    """
    if loss_kind not in _LOSS_FNS and loss_kind != "static_width":
        raise ValueError(
            f"loss_kind must be one of {{integrated_p, cd_variance, "
            f"static_width}}; got {loss_kind!r}"
        )
    if loss_kind == "static_width":
        if alpha is None:
    ...
    for epoch in range(n_epochs):
        ...
        with torch.no_grad():
            ...
            val_loss_t = _compute_loss_legacy(
                model=model, batch=batch,
                scheme_name=scheme_name, statistic_name=statistic_name,
                loss_kind=loss_kind, alpha=alpha,
                n_grid=theta_grid_n, search_mult=search_mult,
