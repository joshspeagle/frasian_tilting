...
def test_smoothed_pvalue_is_strictly_conservative():
    ...
    is correct: at alpha=0.05 with 200 H_0 datasets per setup,
    empirical coverage should be >= 1-alpha minus a small MC margin.
    ...
    theta_true = 0.4
    n_obs = 30
    n_reps = 60  # keep runtime moderate; tolerance is wide
    alpha = 0.10
    n_in = 0
    for _ in range(n_reps):
        ...
        try:
            ...
            n_in += 1  # treat bracket failure as a (conservative) cover
            continue
        if lo <= theta_true <= hi:
            n_in += 1
    cov = n_in / n_reps
